...
namespace JanSordid::ExampleDataModel
{
	...
	struct Entity0 {
		...
		void     TargetingNeedsReset(const f32 targetRange, DynArray<Self> & es );
		template <bool DoEarlyExit>
		void     Targeting(const f32 targetRange, DynArray<Self> & es );
	...
	extern template void Entity0::Targeting<false>(const f32 TargetRange, DynArray<Self> & es );
	extern template void Entity0::Targeting<true> (const f32 TargetRange, DynArray<Self> & es );
}
